...
%=However, considering the state of the cooling infrastructure in combination with the outside temperature potential cost savings of a migration could be mitigated by increased load and cooling expenses.= 
...
%The results should show exactly which measures need to be taken to reduce energy costs with respect to time, energy and workload. 
%The framework should provide the ability to adjust these parameters to show the results under various preconditions. For example, by adjusting the time frame the workload and energy price should accommodate those changes. 
