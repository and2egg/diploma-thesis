...
Suppose a job runs on multiple VMs (possible ???), and it is migrated in some point in time. Considering all servers and data centers
...
\begin{itemize}
	...
	\item Can a server host more than one virtual machine?
	
	\item Can several jobs run on one server? (Depends on CPU requirements)
...
\begin{itemize}
	...
	\item May a job run on multiple VMs (if it needs more CPU power than one machine can provide.. unusual)? 
